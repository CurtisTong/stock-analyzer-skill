...
独立的股票分析 skill 包，提供 7 个股票分析相关 skill + 完整方法论 + 工具脚本。当前同时保留 Codex 可发现的 `.agents/skills/` 和 Claude Code 可安装的 `.claude/skills/` 两套入口。
...
```bash
rm -f ~/.claude/skills/stock ~/.claude/skills/market ~/.claude/skills/sector ~/.claude/skills/portfolio ~/.claude/skills/screener ~/.claude/skills/financial-analyst ~/.claude/skills/investment-researcher
```
