...
A-share 股票分析 Claude Code 插件，提供 9 个核心 skill（`/stock`、`/market`、`/sector`、`/portfolio`、`/screener`、`/monitor`、`/backtest`、`/research`、`/help`）。`/technical` 已合并至 `/stock technical`，`/stock-init` 已合并至 `/screener init`，`/financial-analyst` 和 `/investment-researcher` 已合并至 `/research`。核心脚本零外部依赖（纯 stdlib），数据源为国内 API（腾讯、东方财富、新浪）。
