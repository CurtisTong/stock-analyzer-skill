...
1. **mypy strict 错误清理**：common/ 280+ mypy 错误（`disallow_any_generics` 触发大量裸 `dict`/`list`），本次 PR 通过 `--follow-imports=silent` 仅检查 PR 涉及文件绕过。后续需逐文件加 `dict[K, V]` 类型注解。 ✅ 2026-08-12 完成：**三级清零 + 白名单扩展**——
   ...
   - ✅ 2026-08-13 收尾：白名单扩至 **203 文件（10 目录 + 5 顶层脚本，命令与 ci.yml/pre-commit 一字不差）**——新增 technical/strategies/data/portfolio/monitor/backtest/config/dev 8 目录。过程中**发现并修复 2 个隐藏回归**：① 上一轮把 dcf.py `from macro_indicators import` 改成 `from scripts import macro_indicators` 引入 `macro_indicators` 双身份冲突（`Source file found twice`），使**旧 78 文件 CI 命令直接回归失败**——dcf 回退裸 import、cyclical 改 importlib 延迟加载；② screener `_print_mainline_deviation_warning` 引用的 `SECTOR_ETFS` 在 sector_etf_strength **从未导出**（早已改名），被 `except Exception:` 静默吞掉导致主线偏离警告永久失效——改用 `_load_sector_etfs()`。另清零 experts/（9）：scoring/_utils float 标注、vote_engine 查找字典键 str 窄化、momentum_trader `module_from_spec` None 守卫、calibration `data.get_kline`（PYTHONPATH=scripts 校验）；config/loader value Any；dev/cross_validate_sources `_NotHandled`+set 复用名、experts_cli float 防御；chip/sector result dict 显式标注；portfolio_web 移除死亡 re-export。
    - ✅ 2026-08-13 二次扩展：**`scripts/chan/` 包清零纳入白名单，白名单扩至 212 文件（11 目录 + 5 顶层脚本）**——`beichi.py` result dict 显式 `dict[str, Any]` 标注（原推断 `list[Never]` + `zs.get` 返回类型污染导致 5 错误）。注：顶层 `scripts/chan.py`（兼容 re-export 层）加 `# mypy: disable-error-code="attr-defined"` 头注释后 standalone 检查零错误，但因与 `chan/` 包同名触发 mypy 模块双身份**无法同列一条白名单命令**，逻辑由 `scripts/chan/` 目录扫描覆盖，维持白名单外。
    - ✅ 2026-08-13 边界结论：白名单命令（6 顶层脚本 + 11 目录 = 212 文件）为**架构最优**——验证发现：追加全部 21 个顶层 CLI 中，quote/screener/stock/chip/sector 等 13 个可共存（215 文件全绿），但 `scripts/technical.py`/`scripts/backtest.py`/`scripts/chan.py` 与各自同名包目录（technical//backtest//chan/）并传时触发 `Duplicate module named` mypy 双身份冲突；这些文件是各自目录逻辑的 CLI 别名薄壳，其逻辑已被目录扫描覆盖，因此维持 212 文件边界（验证于 2026-08-13）。
...
| **P2-P1**（NEW） | P2 | portfolio | `PortfolioManager` 41 方法 god class 拆分（crud/oplog/analytics/rebalance 4 模块 + facade） | ✅ 2026-08-12 全部完成（4/4）：v1.16.0 已拆 analytics/rebalance thin wrapper + OpLog 接入 manager；本次新增 `portfolio/crud.py`——全部 10 个 CRUD 写操作方法（add/reduce/remove/update/tag/untag + add/remove_watch）及内部辅助 `_record_trade_log`/`_position_cost` 整体搬移，manager 对应方法变一行委托（-297 行），逻辑零改动（185 项 portfolio 测试通过） |
...
| **P2-P5**（NEW）| P2 | common | 5 维度补充治理（凭据/.env.example 文档化 + 依赖 lockfile + pre-commit 5 hook + mypy 分阶段扩 strict） | ✅ v1.16.0 完成（pre-commit + mypy strict 已再追 212 文件白名单）/ 📋 2026-08-13 .env.example 已补全 `STOCK_DEBUG`+`STOCK_SCREENER_DEADLINE`（16 个消费 env 变量全部声明，审计 §D-A 闭环；.env.example 被 `.gitignore:16 .env.*` 排除不入库，本地生效）。lockfile：项目运行时零外部依赖（stdlib + PyYAML），可选依赖运行期自动检测缺失静默跳过，lockfile ROI 低，维持不引入 |
