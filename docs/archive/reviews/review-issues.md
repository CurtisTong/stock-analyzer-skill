...
> - **Round 11（架构债收官 2026-07-10）**：架构审查（[archive/reviews/architecture-review-2026-07-07.md](archive/reviews/architecture-review-2026-07-07.md)）剩余 9 项 T/I 全部清零，2725 tests passed / ruff 0 errors / 25 skipped：
...
1. **mypy strict 错误清理**：common/ 280+ mypy 错误（`disallow_any_generics` 触发大量裸 `dict`/`list`），v2.5–v2.7 周期通过 `--follow-imports=silent` 仅检查 PR 涉及文件绕过。后续需逐文件加 `dict[K, V]` 类型注解。 ✅ 2026-08-12 完成：**三级清零 + 白名单扩展**——
   ...
   - ✅ v1.20.x 白名单扩至 203 文件（10 目录 + 5 顶层脚本，命令与 ci.yml/pre-commit 一字不差），新增 technical/strategies/data/portfolio/monitor/backtest/config/dev 8 目录。
   - ✅ 修复 2 项回归：① `dcf.py` 存在 `macro_indicators` 双身份冲突（`Source file found twice`），dcf 回退裸 import + cyclical 改 importlib 延迟加载；② `screener._print_mainline_deviation_warning` 引用的 `SECTOR_ETFS` 在 `sector_etf_strength` 未导出（原 `except Exception:` 静默吞掉导致主线偏离警告永久失效），改用 `_load_sector_etfs()`。
   - ✅ 清零 experts/（9 项）：`scoring/_utils` float 标注、`vote_engine` 查找字典键 str 窄化、`momentum_trader` `module_from_spec` None 守卫、`calibration` `data.get_kline`（PYTHONPATH=scripts 校验）；`config/loader` value Any；`dev/cross_validate_sources` `_NotHandled`+set 复用名、`experts_cli` float 防御；`chip/sector` result dict 显式标注；`portfolio_web` 移除死亡 re-export。
    ...
    - ✅ 2026-08-13 边界结论：白名单命令（6 顶层脚本 + 11 目录 = 212 文件）为**架构最优**——验证发现：追加全部 21 个顶层 CLI 中，quote/screener/stock/chip/sector 等 13 个可共存（215 文件全绿），但 `scripts/technical.py`/`scripts/backtest.py`/`scripts/chan.py` 与各自同名包目录（technical//backtest//chan/）并传时触发 `Duplicate module named` mypy 双身份冲突；这些文件是各自目录逻辑的 CLI 别名薄壳。
