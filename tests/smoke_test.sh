#!/usr/bin/env bash
# 端到端冒烟测试：5 个脚本 + 3 个 API 端点 + 6 个 skill + 可选 symlink
set -e
...
echo "==> 1. scripts/ 完整性"
for f in common.py quote.py finance.py kline.py announcements.py; do
  if [ -x "$SCRIPTS/$f" ]; then
    ok "$f 可执行"
  else
    ko "$f 缺失或无执行权限"
...
echo "==> 4. 6 个本地 skill 定义"
for s in stock market sector portfolio financial-analyst investment-researcher; do
...
echo "==> 5. 6 个 symlink 已注册（未安装时可失败）"
for s in stock market sector portfolio financial-analyst investment-researcher; do
...
for s in stock market sector portfolio; do
