...
@given(
    score=st.floats(min_value=70, max_value=100, allow_nan=False, allow_infinity=False)
)
...
@given(
    score=st.floats(min_value=0, max_value=30, allow_nan=False, allow_infinity=False)
)
@settings(max_examples=100)
...
def test_confidence_index_range(
    ...
    assert (
        0 <= result <= 100
    ), f"result={result} (scores={scores}, composite={composite}, cal={cal_factor})"
...
def test_confidence_index_high_composite(scores: list[float]):
    """高综合分 + 零校准 → 信心指数应该偏高。"""
    result = compute_confidence_index(
        scores, composite_score=90.0, calibration_factor=0.0
    )
    # 高综合分时信心指数至少 > 30
...
def test_confidence_index_high_consistency(scores: list[float]):
    ...
    result = compute_confidence_index(
        uniform_scores, composite_score=same_score, calibration_factor=0.0
    )
    # 一致性满分 + 中高分时信心指数至少 > 30
...
def test_expert_registry_count():
    """v2.4.0：8 legacy + 6 extended + 1 momentum + 1 value_institution = 16。"""
    assert len(EXPERT_REGISTRY) in (8, 14, 15, 16)
...
def test_detect_market_state_range(change_pct: float):
    ...
    ), f"state={result['state']} (change_pct={change_pct})"
