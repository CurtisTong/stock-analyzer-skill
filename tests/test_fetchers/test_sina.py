...
class TestSinaQuoteFetcher:
    ...
    def test_fetch_malformed_response(self):
        """格式错误的响应：返回 None。"""
        with patch("fetchers.sina_quote.http_get_with_headers", return_value=b"not_valid"):
            result = self.fetcher.fetch("sh600519")
        assert result is None
    ...
    def test_fetch_http_error(self):
        """HTTP 错误：异常传播。"""
        with patch("fetchers.sina_quote.http_get_with_headers", side_effect=NetworkError("url", "err", 3)):
...
class TestSinaKlineFetcher:
    ...
    def test_fetch_normal(self):
        """正常响应：解析 K 线数据。"""
        data = [
            {"day": "2025-06-10", "open": "1790.00", "high": "1810.00", "low": "1785.00", "close": "1800.00", "volume": "12345"},
            {"day": "2025-06-11", "open": "1800.00", "high": "1815.00", "low": "1795.00", "close": "1805.00", "volume": "11000"},
        ]
        with patch("fetchers.sina_kline.http_get", return_value=json.dumps(data).encode()):
            result = self.fetcher.fetch("sh600519")
        assert result is not None
        assert len(result) == 2
    ...
    def test_fetch_http_error(self):
        """HTTP 错误：异常传播。"""
        with patch("fetchers.sina_kline.http_get", side_effect=NetworkError("url", "err", 3)):
            with pytest.raises(NetworkError):
                self.fetcher.fetch("sh600519")
    ...
    def test_fetch_preserves_original_fields(self):
        ...
        data = [
            {"day": "2025-06-10", "open": "10.00", "high": "11.00", "low": "9.50", "close": "10.50", "volume": "5000", "extra": "kept"},
        ]
        with patch("fetchers.sina_kline.http_get", return_value=json.dumps(data).encode()):
            result = self.fetcher.fetch("sh600519")
        assert result is not None
