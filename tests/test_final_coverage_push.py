...
class TestAnnouncementsMain:
    def test_main_with_code_json(self, capsys):
        ...
        with (
            patch("sys.argv", ["announcements.py", "sh600519", "-j"]),
            patch(
                "announcements.fetch_announcements", return_value=[{"title": "test"}]
            ),
        ):
            try:
                main()
    ...
    def test_main_reports_json(self, capsys):
        ...
        with (
            patch("sys.argv", ["announcements.py", "sh600519", "reports", "-j"]),
...
class TestScreeningServiceFinal:
    def test_screen_with_codes(self):
        ...
        with (
            patch(
                "business.screening_service.compute_features",
                return_value={
                    "trend": 1,
                    "ret20": 5,
                    "ma10": 10,
            ...
            patch(
                "data.get_quote",
                return_value=MagicMock(price=10, prev_close=9, total_cap=100),
            ),
...
class TestBacktestCliMain:
    ...
    def test_main_with_code(self):
        ...
        with (
            patch("sys.argv", ["backtest.py", "sh600519", "--days", "30"]),
            patch(
