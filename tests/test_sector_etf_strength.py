...
class TestBuildStockSectorCompare:
    def test_full_data(self):
        ...
        with (
            ...
            patch.object(
                sector_etf_strength, "find_sector_by_code", return_value=["消费"]
            ),
        ):
    ...
    def test_no_matched_etf(self):
        """个股板块无 ETF 代理。"""
        meta = [
            {"code": "sh510300", "name": "宽基", "category": "宽基", "bk_code": None}
        ]
        ...
        with (
            ...
            patch.object(
                sector_etf_strength, "find_sector_by_code", return_value=["机器人"]
            ),
        ):
            result = sector_etf_strength.build_stock_sector_compare(
                "sh000000",
...
class TestComputeRotationStrength:
    def test_full_data(self):
        ...
        with (
            patch.object(sector_etf_strength, "_load_sector_etfs", return_value=etfs),
            patch.object(
                sector_etf_strength,
                "_fetch_etf_quotes",
                return_value={
    ...
    def test_all_kline_failures(self):
        ...
        with (
            patch.object(sector_etf_strength, "_load_sector_etfs", return_value=etfs),
            patch.object(
                sector_etf_strength, "get_kline", side_effect=Exception("net")
            ),
        ):
            result = sector_etf_strength.compute_rotation_strength(window=5)
...
class TestAnalyze:
    ...
    def test_with_stock_code(self):
        ...
        with (
            ...
            result = sector_etf_strength.analyze(
                stock_code="sh600519", fetch_index=True
            )
...
class TestMain:
    ...
    def test_with_stock_code_json(self, capsys, monkeypatch):
        ...
        with patch.object(
            ...
            monkeypatch.setattr(
                sys, "argv", ["sector_etf_strength.py", "sh600519", "-j"]
            )
            sector_etf_strength.main()
        captured = capsys.readouterr()
