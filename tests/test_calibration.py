...
class TestCalibrationFactor:
    ...
    def test_perfect_calibration_positive(self):
        ...
        for name in data["experts"]:
            data["experts"][name] = {"events": 10, "correct": 10, "last_updated": "2026-06-01"}
        _save(data)
        factor = compute_calibration_factor()
        assert factor > 0.5
    ...
    def test_poor_calibration_negative(self):
        ...
        data = _load()
        for name in data["experts"]:
            data["experts"][name] = {"events": 10, "correct": 0, "last_updated": "2026-06-01"}
        _save(data)
        factor = compute_calibration_factor()
        assert factor < -0.5
    ...
    def test_mixed_calibration(self):
        ...
        for i, name in enumerate(names):
            if i < 4:
                data["experts"][name] = {"events": 10, "correct": 8, "last_updated": "2026-06-01"}
            else:
                data["experts"][name] = {"events": 10, "correct": 3, "last_updated": "2026-06-01"}
        _save(data)
        factor = compute_calibration_factor()
...
class TestCalibrationReport:
    ...
    def test_report_contains_all_experts(self):
        report = get_calibration_report()
        for name in ["buffett", "lynch", "soros", "duan_yongping",
