...
class TestSnapshot:
    ...
    def test_load_failure(self, monkeypatch, tmp_path):
        """文件不存在时返回空 dict。"""
        monkeypatch.setattr(macro_indicators, "SNAPSHOT_PATH", tmp_path / "missing.json")
        result = macro_indicators._load_snapshot()
        assert result == {}
    ...
    def test_is_fresh_recent(self, monkeypatch, tmp_path):
        """新 snapshot 视为 fresh。"""
        f = tmp_path / "macro_snapshot.json"
        f.write_text(json.dumps({"updated": datetime.now().isoformat(timespec="seconds")}))
        monkeypatch.setattr(macro_indicators, "SNAPSHOT_PATH", f)
        assert macro_indicators._snapshot_is_fresh({"updated": datetime.now().isoformat()}) is True
...
class TestFetchTreasury10y:
    def test_from_fresh_snapshot(self, monkeypatch):
        """从 fresh snapshot 读。"""
        with patch.object(macro_indicators, "_load_snapshot",
    ...
    def test_from_yfinance(self, monkeypatch):
        ...
        with patch.object(macro_indicators, "_load_snapshot", return_value={}), \
...
class TestFetchMarginTotal:
    def test_success(self, monkeypatch):
        with patch.object(macro_indicators, "_load_snapshot",
                          return_value={"margin_balance_total_yi": 15000.0,
                                       "margin_change_5d_pct": 2.5, "updated": "now"}), \
             patch.object(macro_indicators, "_snapshot_is_fresh", return_value=True):
...
class TestFetchErpSh300:
    ...
    def test_calculated(self, monkeypatch):
        ...
        with patch.object(macro_indicators, "_load_snapshot", return_value={}), \
             patch.object(macro_indicators, "_snapshot_is_fresh", return_value=False), \
             patch.object(macro_indicators, "fetch_treasury_10y",
...
class TestFetchAll:
    def test_full_success(self, monkeypatch):
        """全部 fetch 成功。"""
        fake_data = {"value": 1.0, "source": "fixture", "unit": "x", "as_of": "2026-07-10"}
        with patch.object(macro_indicators, "fetch_treasury_10y", return_value=fake_data), \
             ...
             patch.object(macro_indicators, "fetch_margin_total",
                          return_value={"value": 15000.0, "change_5d_pct": 2.5,
                                        "as_of": "2026-07-10", "source": "fixture"}), \
             patch.object(macro_indicators, "fetch_if_basis",
                          return_value={"value": 5.0, "unit": "pts", "as_of": "2026-07-10"}), \
             patch.object(macro_indicators, "fetch_ic_basis",
                          return_value={"value": 3.0, "unit": "pts", "as_of": "2026-07-10"}), \
             patch.object(macro_indicators, "fetch_ih_basis",
                          return_value={"value": 4.0, "unit": "pts", "as_of": "2026-07-10"}), \
             patch.object(macro_indicators, "fetch_erp_sh300",
    ...
    def test_partial_failure(self, monkeypatch):
        ...
        with patch.object(macro_indicators, "fetch_treasury_10y", return_value=None), \
...
class TestMain:
    ...
    def test_with_json_flag(self, capsys, monkeypatch):
        with patch.object(macro_indicators, "fetch_all", return_value={
            "macro": {}, "leverage": {}, "valuation_bridge": {},
    ...
    def test_table_output(self, capsys, monkeypatch):
        with patch.object(macro_indicators, "fetch_all", return_value={
            "as_of": "2026-07-10",
            "macro": {
                "treasury_10y_pct": 2.5, "vix": 18.0, "usd_index": 103.0,
                "usd_cny": 7.2, "gold_usd_oz": 2000.0, "brent_oil_usd": 85.0,
                "wti_oil_usd": 80.0, "lithium_carbonate_cny_t": 100000.0,
            },
            "leverage": {
                "margin_balance_total_yi": 15000.0, "margin_change_5d_pct": 2.5,
                "if_main_basis_pts": 5.0, "ic_main_basis_pts": 3.0, "ih_main_basis_pts": 4.0,
            },
