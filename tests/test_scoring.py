...
from experts import EXPERT_REGISTRY, get_expert, list_long_term_experts, list_short_term_experts
...
GOOD_STOCK = {
    ...
    "finance": {
        "ROEJQ": 25, "PARENTNETPROFITTZ": 30, "TOTALOPERATEREVETZ": 20,
        "XSMLL": 60, "ZCFZL": 25, "EPSJB": 2.0, "MGJYXJJE": 2.5,
    },
...
BAD_STOCK = {
    ...
    "finance": {
        "ROEJQ": 2, "PARENTNETPROFITTZ": -20, "TOTALOPERATEREVETZ": -10,
        "XSMLL": 5, "ZCFZL": 85, "EPSJB": -0.5, "MGJYXJJE": -0.3,
    },
    ...
    "market_features": {
        "limit_up_count": 10, "limit_down_count": 60,
        "advance_ratio": 0.15, "break_rate": 0.7,
        "limit_up_30d_count": 0, "sector_limit_up_count": 0,
    },
}
    ...
    def test_good_stock_direction(self):
        ...
        for e in long_experts:
            ...
            if result["score"] >= 60:
                buy_count += 1
        # 长线专家中至少一半应看多
        assert buy_count >= 2, f"Only {buy_count}/4 long-term experts bullish on good stock"
...
class TestExpertDivergence:
    def test_experts_diverge_on_different_stocks(self):
        ...
        hot_stock = {
            ...
            "finance": {"ROEJQ": 5, "PARENTNETPROFITTZ": 10, "ZCFZL": 60,
                        "EPSJB": 0.1, "MGJYXJJE": 0.05},
            "kline_features": {"trend": 1, "rsi": 60, "macd_signal": 1},
            "market_features": {
                "limit_up_count": 90, "limit_down_count": 3,
                "advance_ratio": 0.7, "break_rate": 0.15,
                "limit_up_30d_count": 5, "sector_limit_up_count": 4,
            },
        }
        long_avg = sum(
    ...
    def test_yangjia_dominates_sentiment(self):
        ...
        # 养家在冰点应该情绪维度给高分
        assert result["dim_scores"].get("情绪", 0) >= 80, (
    ...
    def test_buffett_values_roe(self):
        ...
        profile = get_expert("buffett")
        result = score_expert_precise(profile, GOOD_STOCK)
        assert result["dim_scores"]["基本面"] >= 75, (
    ...
    def test_xu_xiang_requires_limit_up(self):
        ...
        assert result["dim_scores"]["技术面"] == 0, (
...
class TestThresholdBoundaries:
    ...
    @pytest.mark.parametrize("cap,expected", [
        (80, 100), (30, 100), (150, 100), (151, 60),
        (300, 60), (501, 0), (29, 0),
    ])
    def test_xu_xiang_valuation_cap(self, cap, expected):
        from experts.scoring import _score_xu_xiang
        result = _score_xu_xiang({"quote": {"circulating_cap": cap}})
        assert result["估值"] == expected

    @pytest.mark.parametrize("limit_up,limit_down,break_rate,expected", [
        (5, 60, 0.7, 100),  # 冰点
        (100, 3, 0.15, 80),  # 主升
        (50, 15, 0.3, 50),   # 震荡
        (10, 40, 0.5, 0),    # 退潮
    ...
    def test_yangjia_sentiment_cycle(self, limit_up, limit_down, break_rate, expected):
        from experts.scoring import _score_chaogu_yangjia
        result = _score_chaogu_yangjia({
            "market_features": {
