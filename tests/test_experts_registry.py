...
class TestRegistry:
    ...
    def test_ensure_loaded(self):
        ...
        with patch(
            "experts.yaml_loader.load_all_experts",
            return_value={
                "buffett": SimpleNamespace(name="buffett", active=True),
                ...
                "sector_specialist": SimpleNamespace(
                    name="sector_specialist", active=True
                ),
...
class TestTopicLeader:
    ...
    def test_score_callable(self):
        """score 函数可调用。"""
        score_fn = getattr(topic_leader, "score", None) or getattr(
            topic_leader, "evaluate", None
        )
        if score_fn:
...
class TestInstitution:
    ...
    def test_score_callable(self):
        score_fn = getattr(institution, "score", None) or getattr(
            institution, "evaluate", None
        )
        if score_fn:
            assert callable(score_fn)
