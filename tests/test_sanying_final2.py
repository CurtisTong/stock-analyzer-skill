...
class TestSanyingPositive:
    def test_three_red_one_green(self):
        ...
        for i in range(n):
            if i < n - 4:
                records.append({"day": f"2026-01-{i+1:02d}", "open": 10, "close": 10, "high": 10, "low": 10})
            elif i < n - 1:
                records.append({"day": f"2026-01-{i+1:02d}", "open": 10, "close": 15, "high": 15, "low": 10})
            else:
                records.append({"day": f"2026-01-{i+1:02d}", "open": 15, "close": 8, "high": 15, "low": 8})
        volumes = [1000] * n
        volumes[-1] = 5000
