...
class TestInferExchange:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("688981", "sh"),  # 科创板
            ("510500", "sh"),  # 上证 ETF
            ("000001", "sz"),  # 无前缀 00 开头 -> 默认深市
            ("300750", "sz"),  # 创业板
            ("159915", "sz"),  # 深证 ETF
            ("430047", "bj"),  # 北交所
            ("sh600519", "sh"),  # 已带正确前缀
            # 上交所指数：00 开头二义段，回退信任入参前缀（修复核心场景）
            ("sh000001", "sh"),  # 上证指数
            ("sh000016", "sh"),  # 上证50
            ("sh000300", "sh"),  # 沪深300
            ("sh000905", "sh"),  # 中证500
            ("SH000001", "sh"),  # 大写前缀同样回退
            ("sz000001", "sz"),  # 深市主板 000001 平安银行，入参 sz 保留
            # 002/003 深市主板号段（原中小板）：非指数号段，误冠 sh 纠正为 sz（P0-02）
            ("SH002335", "sz"),  # 入参 sh -> 纠正为 sz
            ("sh002335", "sz"),  # 同上（小写）
            ("sz002335", "sz"),  # 入参 sz 保留
            ("sz600519", "sh"),  # 沪市股误冠深市前缀 -> 无二义数字段纠正
            # 入参去噪（大写 / 空格 / 换行 / 混合大小写）
            ("  sh000001  ", "sh"),  # 前后空格
            ("Sh000001", "sh"),  # 首字母大写
            ("sh000001\n", "sh"),  # 尾部换行
...
class TestNormalizeQuoteCode:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("600519", "sh600519"),
            ("sh600519", "sh600519"),
            ("000001", "sz000001"),  # 无前缀 -> 默认深市
            ("430047", "bj430047"),
            # 002/003 深市主板号段误冠 sh -> 纠正为 sz（P0-02 回归）
            ("sh002920", "sz002920"),
            ("002920", "sz002920"),
            # 上交所指数保留 sh 前缀（修复核心场景）
            ("sh000001", "sh000001"),
            ("sh000300", "sh000300"),
            ("sh000905", "sh000905"),
