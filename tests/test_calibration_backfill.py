...
class TestCmdStatus:
    ...
    def test_status_with_data(self, capsys):
        from calibration_backfill import cmd_status
        args = MagicMock()
        pending = [{"stock": "茅台", "date": "2026-01-01", "direction": "看多", "verify_after": "2026-01-08"}]
        cal = {"buffett": {"events": 10, "correct": 7, "last_updated": "2026-01-01"}}
        with patch("calibration_backfill.get_pending_predictions", return_value=pending), \
...
class TestCmdVerify:
    def test_verify_no_price(self, capsys):
        ...
        args.days = 30
        args.verbose = False
        with patch("calibration_backfill.verify_predictions", return_value={
            "verified": 0, "updated_experts": 0, "details": [], "skipped": 0,
        }):
            cmd_verify(args)
            captured = capsys.readouterr()
    ...
    def test_verify_with_details(self, capsys):
        ...
        args.verbose = True
        with patch("calibration_backfill.verify_predictions", return_value={
            "verified": 2, "updated_experts": 1, "skipped": 1,
            "details": [
                {"stock": "茅台", "direction": "看多", "correct": True, "actual_return": 5.2},
                {"stock": "平安", "direction": "看空", "correct": False, "actual_return": -3.1},
