...
class TestUpdatePosition:
    def test_update_fields(self, tmp_path):
        ...
        result = mgr.update_position("sh600519", cost=110, quantity=200, name="贵州茅台", buy_date="2025-01-01", tags=["白酒"])
...
class TestTagUntagPosition:
    ...
    def test_tag_position_appends(self, tmp_path):
        mgr = _make_portfolio(tmp_path, initial={
            "version": 2,
            "positions": [{"code": "sh600519", "name": "茅台", "cost": 100, "quantity": 100, "tags": ["白酒"]}],
            "watchlist": [],
        })
    ...
    def test_untag_position(self, tmp_path):
        mgr = _make_portfolio(tmp_path, initial={
            "version": 2,
            "positions": [{"code": "sh600519", "name": "茅台", "cost": 100, "quantity": 100, "tags": ["白酒", "龙头"]}],
            "watchlist": [],
        })
...
class TestWatchOperations:
    ...
    def test_add_watch_updates_existing(self, tmp_path):
        mgr = _make_portfolio(tmp_path, initial={
            "version": 2, "positions": [],
            "watchlist": [{"code": "sz000858", "name": "", "target_buy": 0, "target_sell": 0}],
        })
    ...
    def test_get_all_codes(self, tmp_path):
        mgr = _make_portfolio(tmp_path, initial={
            "version": 2,
            "positions": [{"code": "sh600519", "name": "茅台", "cost": 100, "quantity": 100, "tags": []}],
            "watchlist": [{"code": "sz000858", "name": "五粮液", "target_buy": 0, "target_sell": 0}],
        })
...
class TestCheckConcentration:
    ...
    def test_cost_basis(self, tmp_path):
        mgr = _make_portfolio(tmp_path, initial={
            "version": 2,
            "positions": [
                {"code": "sh600519", "name": "茅台", "cost": 100, "quantity": 100, "tags": ["白酒"]},
                {"code": "sh600001", "name": "浦发", "cost": 10, "quantity": 100, "tags": ["银行"]},
            ],
            "watchlist": [],
        })
        result = mgr.check_concentration()
    ...
    def test_industry_concentration_warning(self, tmp_path):
        mgr = _make_portfolio(tmp_path, initial={
            ...
            "positions": [
                {"code": "sh600519", "name": "茅台", "cost": 100, "quantity": 100, "tags": ["白酒"]},
                {"code": "sz000858", "name": "五粮液", "cost": 100, "quantity": 100, "tags": ["白酒"]},
            ],
            "watchlist": [],
        })
    ...
    def test_top3_concentration(self, tmp_path):
        mgr = _make_portfolio(tmp_path, initial={
            "version": 2,
            "positions": [
                {"code": f"sh60000{i}", "name": f"S{i}", "cost": 100, "quantity": 100, "tags": []}
                for i in range(4)
            ],
            "watchlist": [],
        })
    ...
    def test_zero_total_value(self, tmp_path):
        mgr = _make_portfolio(tmp_path, initial={
            "version": 2,
            "positions": [{"code": "sh600519", "name": "茅台", "cost": 0, "quantity": 0, "tags": []}],
            "watchlist": [],
        })
        result = mgr.check_concentration()
        assert result == {"warnings": [], "details": {}}
...
class TestRiskSummary:
    ...
    def test_with_positions(self, tmp_path):
        ...
        with patch("business.risk_metrics.position_var_summary", return_value=fake_result):
