...
class TestRecordAll:
    ...
    def test_records_all_strategies(self, tmp_path):
        """正常记录所有策略。"""
        with patch("strategy_performance.load_test_universe", return_value=["sh600519"]), \
    ...
    def test_custom_codes_used(self):
        ...
        with patch("strategy_performance.load_test_universe") as mock_load, \
             patch("strategy_performance.run_backtest", return_value=_backtest_result()), \
             patch("strategy_performance._load", return_value={"records": []}), \
             patch("strategy_performance._save"):
...
class TestLoadSave:
    ...
    def test_load_existing_file(self, tmp_path, monkeypatch):
        ...
        f.write_text(
            json.dumps({"records": [{"month": "2026-06"}]}), encoding="utf-8"
        )
...
class TestReportExtra:
    def test_latest_record_returned(self):
        records = {
            "records": [
                {"month": "2026-05", "strategies": {"balanced": {"total_return_pct": 5.0}}},
                {"month": "2026-06", "strategies": {"balanced": {"total_return_pct": 10.0}}},
            ]
        }
        with patch("strategy_performance._load", return_value=records):
            result = report()
...
class TestMain:
    def test_main_record(self, capsys):
        with patch("sys.argv", ["strategy_performance.py", "record", "--codes", "sh600519", "--days", "30"]), \
             patch("strategy_performance.record_all", return_value={
                 "strategies": {"balanced": {"total_return_pct": 10.0, "sharpe_ratio": 1.5, "win_rate_pct": 60.0}}
             }):
    ...
    def test_main_record_error(self, capsys):
        with patch("sys.argv", ["strategy_performance.py", "record"]), \
    ...
    def test_main_report(self, capsys):
        report_result = {
            "by_month": {
                "2026-06": {
                    "balanced": {"runs": 2, "total_return_pct": 10.0, "sharpe_ratio": 1.5, "win_rate_pct": 60.0}
                }
            },
            "latest": {"month": "2026-06"},
        }
        with patch("sys.argv", ["strategy_performance.py", "report"]), \
    ...
    def test_main_report_json(self, capsys):
        ...
        with patch("sys.argv", ["strategy_performance.py", "report", "-j"]), \
             patch("strategy_performance.report", return_value=report_result):
            from strategy_performance import main
            main()
        out = capsys.readouterr().out
    ...
    def test_main_compare(self, capsys):
        compare_result = {
            "metric": "sharpe_ratio",
            "ranking": [{"strategy": "balanced", "label": "均衡", "value": 1.5, "runs": 2}],
            "best": "balanced",
            "worst": "balanced",
            "spread": 0,
        }
        with patch("sys.argv", ["strategy_performance.py", "compare"]), \
    ...
    def test_main_compare_json(self, capsys):
        ...
        with patch("sys.argv", ["strategy_performance.py", "compare", "-j"]), \
             patch("strategy_performance.compare", return_value=compare_result):
            from strategy_performance import main
            main()
        out = capsys.readouterr().out
