...
def running_server(tmp_path: Path):
    """启动 ThreadingHTTPServer 在空闲端口，yield base_url 与临时数据文件路径。

    用法::

        with running_server(tmp_path) as (url, data_file):
            ...
    """
...
class TestRouting:
    ...
    def test_list_positions_populated(self, tmp_path):
        ...
        t.start()
        try:
            status, body = _hit(f"http://127.0.0.1:{port}/api/positions")
            assert status == 200
            j = json.loads(body)
    ...
    def test_get_one_found(self, tmp_path):
        with running_server(tmp_path) as (base, _):
            ...
            status, body = _hit(f"{base}/api/positions/sh600989")
            assert status == 200
            j = json.loads(body)
            assert j["ok"]
    ...
    def test_get_one_not_found_404(self, tmp_path):
        with running_server(tmp_path) as (base, _):
            status, body = _hit(f"{base}/api/positions/nope")
            assert status == 404
            j = json.loads(body)
            assert not j["ok"]
    ...
    def test_wrong_method_405(self, tmp_path):
        with running_server(tmp_path) as (base, _):
            status, body = _hit(f"{base}/api/positions", "DELETE")
            assert status == 405
            j = json.loads(body)
    ...
    def test_post_empty_body_400(self, tmp_path):
        with running_server(tmp_path) as (base, _):
            status, body = _hit(f"{base}/api/positions", "POST", raw_body=b"")
            assert status == 400
            j = json.loads(body)
...
class TestActions:
    ...
    def test_missing_action_400(self, tmp_path):
        with running_server(tmp_path) as (base, _):
            status, body = _hit(f"{base}/api/positions", "POST", {"code": "sh600989"})
            assert status == 400
...
class TestConcurrency:
    def test_concurrent_add_positions(self, tmp_path):
        ...
        with running_server(tmp_path) as (base, _):
            ...
            s, b = _hit(f"{base}/api/positions/sh688888")
            j = json.loads(b)
            assert j["data"]["position"]["quantity"] == 50

            # sh600000..sh600009 应有 10 条
            s, b = _hit(f"{base}/api/positions")
            j = json.loads(b)
...
class TestEndToEnd:
    def test_full_crud_lifecycle(self, tmp_path):
        with running_server(tmp_path) as (base, data_file):
            # 1. add
            s, b = _hit(f"{base}/api/positions", "POST", {
            ...
            assert s == 200
            # 2. read
            s, b = _hit(f"{base}/api/positions/sh600989")
...
class TestBodyLimit:
    def test_body_too_large(self, tmp_path):
        with running_server(tmp_path) as (base, _):
            # 9 KB > 8 KB limit
            huge = b"x" * (9 * 1024)
            s, b = _hit(f"{base}/api/positions", "POST", raw_body=huge)
            # urllib 默认发 application/x-www-form-urlencoded 之外的 → 415 优先
            # 我们强制 application/json 走 size 检查
