...
class TestLiquidityByCapTier:
    ...
    def test_large_cap_normal_turnover(self):
        ...
        quote = {"code": "sh600519", "amount": 100000_0000,
                 "total_cap": 1000, "turnover": 1.0}
    ...
    def test_large_cap_high_turnover(self):
        ...
        quote = {"code": "sh600519", "amount": 10000_0000,
                 "total_cap": 140, "turnover": 7.0}
        score = liquidity.liquidity_score(quote)
        # 不应封顶 100（换手率分支差异）
        assert score < 100
    ...
    def test_small_cap_extreme_turnover(self):
        """小盘股换手率 > 20% → +6（验证触发条件）。"""
        quote = {"code": "sh600519", "amount": 100000_0000,
                 "total_cap": 50, "turnover": 25.0}
        score_extreme = liquidity.liquidity_score(quote)
        # 与正常换手率对比
        quote_normal = {"code": "sh600519", "amount": 100000_0000,
                        "total_cap": 50, "turnover": 5.0}
    ...
    def test_other_board_fallback(self):
        """非标准板块用默认阈值。"""
        quote = {"code": "xx1234", "amount": 100000_0000,
                 "total_cap": 100, "turnover": 1.0}
        score = liquidity.liquidity_score(quote)
        assert score > 0
