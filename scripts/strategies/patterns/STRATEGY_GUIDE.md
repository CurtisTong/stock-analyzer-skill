...
> 本文档是策略的**详细使用手册**（参数、信号解读、实战建议）。
> 模块结构索引见 [README.md](./README.md)。
...
**回测结果**（⚠️ 样本内拟合，5 只股票 × 200 个交易日，未做外样本验证）：

- 胜率：⚠️ 样本内 71.4%（仅单股最优），5 只平均 59.7%
- 平均收益：+6.39%（⚠️ 样本内）
- 累计收益：+44.71%（⚠️ 样本内）
- ⚠️ **重要**：小样本过拟合，实际表现可能远差于回测，不构成投资建议
...
**MA10/MA21 + 放量 2.5x 组合策略** 在 5 只股票 + 50+ 参数组合 + 200+ 笔交易**样本内**回测中表现最优：

- 胜率：60-70%（样本内）
- 平均收益：+3-5%（样本内）
- 止损：-5%
- 持有期：5 天
- ⚠️ 未经外样本/基准对比验证，统计显著性不足，数字仅供参考
