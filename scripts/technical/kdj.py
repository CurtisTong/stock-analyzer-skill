...
def kdj_full(closes, highs, lows, n=9, board="主板"):
    ...
    冷启动与通达信/同花顺的已知偏差（P2-10 标注，TODO(v2.0) 增加 warmup 参数）：
    - K/D 初值硬编码 50，前 n-1 根仅用于预热收敛，第 n 根开始输出 K/D。
    - 通达信/同花顺通常需要更长历史（如 100+ 根）才能完全收敛，本实现在前 9 根
      可能与第三方软件有 1-3 单位偏差，偏差随历史延长而消失。
    - 对短线交易信号（超买/超卖/钝化）影响有限；对长周期回测的初始阶段影响 <5%。
    """
    if len(closes) < n + 1:
        return None
