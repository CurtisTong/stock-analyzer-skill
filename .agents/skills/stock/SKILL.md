---
name: stock
description: 单股分析 skill。用于 A 股个股快速分析、完整五层分析、估值/技术面/板块/风险收益比判断，以及 9 人专家圆桌多空讨论（6 长线 + 3 短线）；优先使用 stock-analyzer-skill 包内 scripts/ 工具获取实时行情、财务、K 线、公告和研报数据。
---
...
- `debate`（默认全模式）：五层分析 + 9 人专家圆桌多空辩论（6 长线 + 3 短线）
...
| 模式           | 参数          | 参与专家           | 决策机制               |
| -------------- | ------------- | ------------------ | ---------------------- |
| 全模式（默认） | `debate`      | 9 人（长线 6+短线 3）| 跨组加权，市场环境调权 |
