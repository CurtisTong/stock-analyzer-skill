...
**报告质量约束**（审查 #27,28,29,30）：
- **去重**：同一数据点（如 OCF/净利润比、ROE）全文最多出现 2 次（核心数据表 1 次 + 分析论证 1 次），禁止 3 次以上重复。
- **评分统一**：全文统一 `A+/A/B+/B/C` 字母制，禁止 ⭐ 符号与字母混用。
- **合并表格**：营收/净利/盈利三张表合并为"核心财务数据表"，其他章节只引用增量信息。
- **排名限定**：禁止跨行业排名（vs 茅台/长电/神华），排名限定本行业内。
- **数据来源**：核心数据表每行须标注来源（年报/季报/预增公告/研报API），禁止推算值不标来源。
