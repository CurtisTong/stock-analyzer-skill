...
| 情绪        | **30%** | 资金流向、舆论一致性、政策预期   | 主流观点极度悲观但基本面改善→100分，高度一致看多→0分          |
...
- 流动性枯竭（两市日均成交额<7000亿且持续缩量，阈值可通过 `scoring.yaml` 的 `experts.soros.market_liquidity_floor_yi` 配置）
- 政策明确打压该板块（反身性向下强化）
...
- **反身性**：陈述"反身性正/负循环"必须引用趋势与情绪数据（如"连涨5日+涨停家数 92 家->反身性正循环"），不得仅凭定性判断。
- **趋势方向**：引用趋势评分须附 K 线数据（如"近5日收盘递增->技术面 100"）。
- **情绪冰点**：声明"冰点机会"必须引用情绪指标数值（如"涨停家数 12 家<阈值->情绪 100"），满足代码门槛。
- **模型边界**：低分仅代表"未形成反身性循环/趋势不明确"，不得推导为"反向指标"或"价格将下跌"。
