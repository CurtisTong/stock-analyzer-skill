...
- **backtest**: P0-11 walk-forward 回测框架 (OOS 验证)
...
### Fixed
- **experts**: 移除 __init__ 顶层 _ensure_loaded 副作用修复测试顺序污染
- **scripts**: 修复 import 路径与 sync_version 正则 named-group 引用
- **experts**: dynamic load data.pool to support multiple import environments
- incremental_ma 契约改 NaN，同步 laoyatou/meirenjian 守卫，修 technical 日志吞噬
- 版本号修正 1.50.0 -> 1.5.0
...
- **business**: P1-18~22 超时/过滤/风控/ST 统一
- **technical**: 前瞻标注 + score 归一化注释 + 信号结构化 (P1-14/15/16)
- **chan**: 中枢 GG/DD 边界 + 注释更新 (P1-12, P1-13)
- **fetcher**: P1-01~05 fetcher 加固
- **ci**: release 标准对齐 + changelog 并发控制 + 修复市场状态回归 (P0-13, P0-14)
- **business**: 填充数据来源元信息 + 修复指数行情传递 (P0-02, P1-17)
- **experts**: 修复 aggregate_group_votes 全空死代码 (P0-09)
- **experts**: YAML 维度名归一化 (P0-06)
- **circuit-breaker**: recovery_timeout <= 0 钳制为 1 (P0-05)
- **fetcher**: 429 限速不计入熔断失败 (P0-04)
- **fetcher**: timeout/retry 配置生效 (P0-03)
- 第五轮审查 P1×1 + P2×2 修复
- 第四轮审查 P1×3 + P2×1 修复
- 第三轮审查 P0×8 + P1×5 修复
...
### Changed
- **strategies**: factors/common.py 重命名为 score_utils.py
- **experts**: 公开化 consistency_from_scores + 移除假循环依赖
- **experts**: P2-01 三源合一为 YAML 单源 (删除硬编码 profile)
- **technical**: P1-15 composite_score 去魔数化 (_SCORE_MAX 提取为模块级常量)
- **business/config**: P2-20~24 长期权重/YAML/服务化/max_drawdown/ConfigLoader/cache shim
- **experts**: P2-01~04 别名清理 + buffett 警示 + momentum ST veto + 三源标注
- **experts**: 删除 apply_veto 死代码 (P1-07)
- **strategies**: 跳过权重为 0 的因子计算 (P0-12)
...
### Documentation
- **stock**: 报告三段式改用语义化命名 + 新增关键数字单一信源护栏
- v2.0.0 里程碑 7/7 全部完成（P2-01 + P2-05 收尾）
- 同步 review-issues.md + ADR-004 标记 Round 10 完成
- **changelog,arch,index**: rewrite + dual-expert beautification
- 更新 review-issues.md 标记 P2 全量修复完成（Round 9）
- **tests/release**: P2-25~30 frontmatter/keywords/ADR/catalog 自动生成/sync_version 重构
- **skills/test/ci**: P1-23~30 catalog 补全 + 去重 + 测试补全 + CI 加固 + 死配置清理
- **experts**: 对齐 decide.md 巴菲特警示表格与代码 (P0-07)
...
### Testing
- 最终覆盖率推进（92.5%->93%+）
- 补充15个覆盖率测试文件（92.3%->94%+）
- 补充10个覆盖率测试文件（92%->93%+）
- 补充15个覆盖率测试文件（91.7%->93%+）
- 补 fetchers/kline/efinance_kline.py 12 tests
- 补 fetchers 多个 fetcher 测试（25 tests）
- 补 value_institution / 本土战法 / quote / monitor / event 测试（55 tests）
- 补 experts/scoring/formatter/decide/chan 测试（67 tests）
- 补 market_anchor.py 剩余未覆盖分支（28 tests）
- 补 filters/__init__.py + experts/registry.py 测试（15 tests）
- 补 backtest.py wrapper 入口测试（2 tests）
- 补 monitor/health.py + init_pool.py 单元测试（25 tests）
- 补 calibration / chip / technical 单元测试（57 tests）
- 补充14个覆盖率测试文件（quality/fetcher/dispatch/patterns/cli/exceptions等）
- 批量新增20+测试文件覆盖率89%->93%+（~800用例）
- 批量新增10个测试文件覆盖率80%->90%+（248用例）
- 批量新增10个测试文件覆盖率75%->80%+（451用例）
- 补 sync_skill_test_versions 单元测试（14 tests）
- 补 finance/quote/monitor_health/long_term 单元测试
- 投资/用户/测试专家协作补 5 大模块单元测试到 90%
- 提交 market_anchor v2.5-v2.7 配套测试（172 用例）
- **e2e**: P1-27 扩展 13 skill E2E 工作流测试 (frontmatter + mock 工作流)
- **experts**: P1-08 4:1 边界测试矩阵
...
### Maintenance
- black + ruff 全量格式化（pre-commit run --all-files 触发）
- **data**: 刷新 sector_stocks.json 半导体板块股票池
- **experts**: black reformat vote_engine.py 多行条件合并
- **docs**: 新增 tests/README_TEST_NAMING.md 说明测试蔓延
- **docs**: patterns 双指针互引用 + 新增 docs/releases v1.15.0
- **ci**: pytest-sprint-tests 加 -m 'not network' filter
- **tests**: 修正 stale 引用 9人/15份人设→8人/16份人设
- **docs**: 修正 stale 计数 fetcher 26/33→27/35 screener 5策略→6策略
- **git**: 取消跟踪 .zcode/plans 本地工作笔记
- **release**: 同步版本号至 v1.15.0
- **scripts**: 刷新脚本目录与板块元数据
- **data**: refresh sector_stocks.json _meta.updated timestamp
- 全部 skill 模型切换到火山 glm-5.2
- **lint**: 清理全部 20 个遗留 ruff 错误 (F841/E741/E714/F821)
- **claude**: 收紧 settings.json 权限白名单 (P0-01)
- **release**: 同步至 v1.14.3
- black 折行（backtest cli/metrics 无逻辑变化）
- **settings**: 合并 IDE 自动学习的权限 allowlist

### Other
- Merge branch 'feat/stock-report-format' into main
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- Merge branch 'feat/value-anchor-optimization'
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- Merge branch 'main' of github.com:CurtisTong/stock-analyzer-skill
- test+chore: 覆盖率提升至75%+ + 版本号统一1.50
- test+chore: 覆盖率 65.7% -> 70.4% + 版本升级 v2.8.0
...
### Documentation
- 同步 v1.16.0 新模块到 CLAUDE/README/developer-guide/api-reference
- **skills**: 破位检测护栏与位置不变量提示
- **workflow**: 同步专家圆桌 15→16 份与 fetcher 28→27
- **claude**: 同步 CLAUDE.md 反映 WP1-WP6 改造
- **claude**: 更新模块说明反映 WP1-WP6 改造
...
### Fixed
- **fetchers**: 修复剩余 3 项 P1（PE 口径 + 单位统一 + 测试覆盖）
- **fetchers**: 深度审查修复 27 项问题（5 P0 + 7 P1 + 8 P2）
- **skills**: 清理 skill schema 配置 + git 跟踪遗留 + 路径修正
- **technical**: 技术模块审查修复 H1-H3/M1-M5/L1-L4 + 单测回归
