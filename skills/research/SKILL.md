...
> ⚠️ **NOTE**：本 skill 无独立 Python 入口（`scripts/research.py` 不存在），由 Claude 通过 `allowed-tools` 委派到 `finance.py` / `quote.py` / `kline.py` / `technical.py` / `market_anchor.py` / `events.py` / `sector.py` / `announcements.py` / `stock.py` 等脚本组合调用。直接运行 `python3 scripts/research.py <args>` 会 ModuleNotFoundError。
...
高净利率 → 护城河/品牌溢价 | 高周转率 → 运营效率型 | 高杠杆 → 风险驱动型（需警惕去杠杆）。
...
个股/行业/市场？时间维度？投资风格（价值/成长/趋势/逆向）？分析深度（快速扫描 / 标准报告 / 深度尽调）？

### Step 2: 获取基础数据（强制 9 项）

> ⚠️ **强制调用**：以下脚本**缺一不可**，遗漏任一项将导致报告置信度自动下调至"低"。

```bash
python3 scripts/quote.py <代码> -j                   # 1. 行情
python3 scripts/finance.py <SH/SZ代码> -j            # 2. 财务
python3 scripts/kline.py <代码> 240 60               # 3. K线
python3 scripts/technical.py <代码> --classify       # 4. 技术分析（指标维度）
python3 scripts/announcements.py <代码> [reports]    # 5. 公告/研报
python3 scripts/events.py <代码> --days 60 -j        # 6. ★ 事件日历（业绩预告/分红/解禁/股东变动）必调
python3 scripts/market_anchor.py --no-portfolio -j   # 7. ★ 市场环境锚定（大盘+板块+宽度）必调
python3 scripts/sector.py <板块名> -j                # 8. ★ 板块强度（行业景气与轮动位置）多股时必调
python3 scripts/stock.py <代码> -j                   # 9. ★ 综合评分（系统评级+买卖信号+5维融合）必调
```

**事件日历规则**：`forecast_type=increase` 且 `change_midpoint > 50%` → 评级上跳候选（**需经信号冲突检测后生效**）；`decrease` 且 `< -30%` → 下调候选；公告日期 < 7 天 → 强催化，需在"核心观点"显著标注。

**市场环境规则**：必须用 `market_anchor.py` 的 `regime` + `breadth.advance_ratio` 联合判断，不可用 `quote.py sh000001` 涨跌替代；`regime=sideways` 时禁止"大盘强势"等乐观定性；查个股 RPS 用 `market_anchor.py <code> --no-portfolio -j`。

**综合评分规则**：`stock.py` 综合评分是**多维度融合的最终结论**，与单维数据可能冲突；单维信号（业绩/事件/技术）与综合评分矛盾时**必须触发信号冲突检测**（见 Guardrails 规则 3.5）；综合评分 < 50 → **禁止评级上跳到 BUY**（除非 3 个以上独立维度强支撑）。

### Step 3: 分模块分析（复用 `/stock full`，不重复实现）

直接调用 [`/stock <代码> full`](../stock/SKILL.md) 取其五层分析输出（符合 `skills/_shared/contracts/stock.schema.json`），仅融合以下增量证据：
...
| 同业基准 | `scripts/quote.py <可比公司代码> -j` | 多股对比必须引用行业龙头 PE/PB 锚 |
| 组合相关性 | `scripts/portfolio_correlation.py -j` | 多股配置权重需基于相关性矩阵 |
...
## 核心观点（3-5 行）
## 事件催化 (event_catalysts)          ★ 必出
## 信号冲突声明 (signal_conflict)     ★ 强制（见 Guardrails 规则 3.5）
...
**signal_conflict 章节**：按 Guardrails 规则 3.5 判定后输出冲突类型/程度/维度明细/评级裁决/触发条件。
...
- **同业基准必引**：估值对比至少引用 1 个行业龙头（如锂电→宁德时代 PE 锚），否则 PE/PB 数字无解读意义。
...
1. **冲突程度 = 高**（2 个以上独立维度同时冲突）：评级**禁止上调**至 BUY（可至 HOLD）；投资建议改为 **"等待确认"**（"中报兑现 + 技术面修复"双确认）；核心观点**显著标注**"业绩 vs 信号冲突"。
2. **冲突程度 = 中**（1 个维度冲突）：评级**可上调一档**但**不上跳到 BUY**；报告给出"冲突说明"章节。
3. **冲突程度 = 低**（仅单一指标轻微冲突）：评级按常规上跳；报告附"风险提示"标注冲突指标。
...
**强制声明**：报告必须包含 `## 信号冲突声明` 章节，判定后给出"无冲突 / 中等冲突 / 高冲突"结论。
